...
console.log('📊 Création des données géographiques inline...');
...
try {
  ...
  const inlineScript = `
// Données géographiques du Bénin (inline)
...
`;
  ...
  // Écrire le script inline
  fs.writeFileSync('./web/geo-data-inline.js', inlineScript);
  
  console.log('✅ Fichier geo-data-inline.js créé avec succès');
  
  // Vérifier la taille
  const stats = fs.statSync('./web/geo-data-inline.js');
  console.log('📊 Taille du fichier inline:', Math.round(stats.size / 1024), 'KB');
