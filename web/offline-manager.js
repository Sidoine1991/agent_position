...
if (typeof OfflineManager === 'undefined') {
  class OfflineManager {
    ...
    async getOfflineData(storeName, filters = {}) {
      // Vérifier et réinitialiser la connexion DB si nécessaire
      if (!this.db || this.db.version === 0) {
        try {
          await this.initDB();
        } catch (e) {
          console.warn('⚠️ Impossible de réinitialiser IndexedDB:', e);
          return [];
        }
      }
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          transaction.onerror = () => {
            console.warn('⚠️ Transaction error:', transaction.error);
            resolve([]);
          };
        } catch (error) {
          console.warn('⚠️ IndexedDB transaction error:', error.message);
          resolve([]);
        }
      });
    }
