...
// Sauvegarder la référence originale de fetch
const originalFetch = window.fetch;

// Créer une version patchée de fetch
window.fetch = function patchedFetch(input, init = {}) {
  try {
    const requestUrl = typeof input === 'string' ? input : input?.url;
    if (requestUrl && ApiClient.shouldAttachAuth(requestUrl)) {
      const token = ApiClient.resolveToken();
      if (token) {
        const baseHeaders =
          init.headers ||
...
/**
 * Fonction wrapper pour les appels API avec gestion automatique du token
 * @param {string} path - Chemin de l'API
 * @param {Object} opts - Options de la requête
...
async function api(path, opts = {}) {
  ...
  // Si ce n'est pas un FormData, on ajoute le Content-Type
  if (!(opts.body instanceof FormData)) {
    headers['Content-Type'] = 'application/json';
  }

  // Récupérer le token actuel
  let currentToken = jwt || getJwtToken();
  
  // Rafraîchir le token si nécessaire
  if (currentToken) {
    const refreshedToken = await window.refreshTokenIfNeeded(currentToken);
    if (refreshedToken) {
      currentToken = refreshedToken;
      jwt = refreshedToken; // Mettre à jour le token global
    }
  ...
  const fetchOptions = {
    ...opts,
    headers,
    // Ajouter un timeout de 30 secondes par défaut
    signal: AbortSignal.timeout(30000)
  };

  // Gérer le corps de la requête
  if (opts.body) {
    fetchOptions.body = opts.body instanceof FormData 
      ? opts.body 
  ...
  console.log(`🌐 API ${opts.method || 'GET'} ${path}`, { 
    ...
    body: opts.body ? (opts.body instanceof FormData ? '[FormData]' : opts.body) : undefined
  });
  ...
  try {
    ...
    if (res.status === 401) {
      console.warn('🔒 401 - Authentification requise');
      showAuthBanner();
    ...
    if (!res.ok) {
      ...
      console.error(`❌ Erreur API ${path}:`, errorData);
      
      // Si le token est invalide, on déconnecte
      if (res.status === 403 || (errorData.error === 'invalid_token')) {
...
function geoPromise() {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      timeout: 30000, // 30 secondes (réduit de 45s pour une meilleure réactivité)
    ...
    const errorHandler = (error, isRetry = false) => {
      const errorMessages = {
        ...
        default: 'Impossible d\'obtenir la position actuelle'
      };
      
      const message = errorMessages[error.code] || errorMessages.default;
      console.warn(`Erreur GPS${isRetry ? ' (2e tentative)' : ''}:`, message);
      
      if (isRetry) {
        reject(new Error(`GPS indisponible: ${message}`));
      }
    };
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      firstError => {
        // En cas d'échec, réessayer avec des paramètres plus permissifs
        options.enableHighAccuracy = false;
        options.timeout = 60000; // 60 secondes pour la deuxième tentative
        
        navigator.geolocation.getCurrentPosition(
          position => resolve(position.coords),
          secondError => errorHandler(secondError, true),
          options
        );
      },
      options
...
// Constantes pour la gestion des tentatives de connexion
const MAX_LOGIN_ATTEMPTS = 3;
let loginAttempts = 0;
let isLoginInProgress = false;
...
async function autoLogin(email, password) {
  ...
  if (isLoginInProgress) {
    ...
    return { success: false, message: 'Une connexion est déjà en cours' };
  }

  if (loginAttempts >= MAX_LOGIN_ATTEMPTS) {
    const errorMsg = '❌ Trop de tentatives de connexion, veuillez réessayer plus tard';
  ...
  try {
    console.log(
      `🔐 Tentative de connexion automatique (${loginAttempts}/${MAX_LOGIN_ATTEMPTS})...`
    );
    ...
    const response = await api('/login', {
      ...
      body: JSON.stringify({ 
        email: email.trim(),
        password, // Le mot de passe doit être haché côté client si nécessaire
      }),
      headers: {
        'Content-Type': 'application/json',
      },
...
async function init() {
  ...
  try {
    ...
    if (existingToken) {
      jwt = existingToken;
      console.log('🔑 JWT trouvé dans le stockage local');
    }
    ...
    // En cas d'erreur, on continue avec un état non authentifié
    jwt = '';
  }

  // Charger les paramètres de l'application en arrière-plan
  const loadAppSettings = async () => {
    try {
      const response = await api('/settings');
      if (response?.success) {
        appSettings = response.settings || {};
        console.log('⚙️ Paramètres de l\'application chargés');
      }
    } catch (error) {
      console.warn('⚠️ Impossible de charger les paramètres:', error.message);
      appSettings = {}; // Utiliser des valeurs par défaut
    }
  };
  
  // Démarrer le chargement des paramètres sans attendre
...
function logout() {
  try {
    ...
    // Effacer la session via le gestionnaire de session
    if (window.sessionManager) {
      window.sessionManager.clearSession();
    }
    
    // Mettre à jour l'état de connexion
    localStorage.setItem('presence_update', JSON.stringify({ 
      type: 'logout', 
      ts: Date.now() 
    }));
    ...
    // Afficher une notification
    try { 
      showNotification('Déconnexion réussie', 'success', 1500); 
    } catch (e) {
...
// Fonction pour afficher une bannière d'authentification
function showAuthBanner() {
  try {
    ...
    if (!banner) {
      ...
      banner.style.cssText = 'margin:12px 0;padding:12px 16px;border-radius:8px;background:#f8d7da;color:#721c24;border:1px solid #f5c6cb;';
      banner.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <span>🔒 Votre session a expiré. Veuillez vous reconnecter.</span>
          <button onclick="window.location.href='/index.html'" style="margin-left:12px;padding:4px 12px;background:#dc3545;color:white;border:none;border-radius:4px;cursor:pointer;">
            Se connecter
          </button>
        </div>
      `;
    ...
    console.error('Erreur lors de l\'affichage de la bannière d\'authentification:', error);
  }
}
...
async function loadDashboardMetrics() {
  try {
    if (!jwt) {
      console.warn('loadDashboardMetrics: JWT non disponible');
      return;
    }
    ...
    try { 
      ...
      console.warn('Erreur chargement profil utilisateur:', error.message);
      return;
    }
    
    const userId = me?.id;
    if (!userId) {
      console.warn('loadDashboardMetrics: userId non disponible');
      return;
    }
    ...
    if (!sbUrl || !sbUrl.startsWith('https://') || !sbKey) {
      console.warn('Configuration Supabase invalide ou manquante:', { 
        hasUrl: !!sbUrl, 
        urlStarts: sbUrl?.startsWith('https://'), 
        hasKey: !!sbKey 
      });
    ...
    try {
      ...
      // Ajouter timeout et retry pour la connexion
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 8000); // 8 secondes timeout
      
      const res = await fetch(`${sbUrl}/rest/v1/users?${p.toString()}`, { 
      ...
      if (res.ok) {
        ...
        console.warn('Erreur de réponse Supabase users:', res.status, res.statusText);
      }
    } catch (error) {
      if (error.name === 'AbortError') {
        console.warn('Timeout de connexion Supabase users');
      } else {
        console.warn('Erreur de connexion Supabase users:', error.message);
      }
      // Continuer sans les données de référence
    }
    ...
    try {
      ...
      // Tentative 1: colonne created_at avec client Supabase JS si disponible
      let res = null;
      if (window.supabase && typeof window.supabase.from === 'function') {
        try {
          const { data: checkinsData, error: checkinsError } = await window.supabase
            .from('checkins')
            .select('id,user_id,lat,lon,created_at')
            .eq('user_id', Number(userId))
            .gte('created_at', fromIso)
            .lte('created_at', toIso)
            .order('created_at', { ascending: false });

          if (!checkinsError && Array.isArray(checkinsData)) {
            checkins = checkinsData;
            res = { ok: true };
          }
        } catch (sbError) {
          console.warn('Erreur requête Supabase JS checkins:', sbError);
        }
      }

      // Fallback: requête REST API sans filtres de date complexes (on filtrera côté client)
      if (!res || !res.ok) {
        ...
        s1.set('limit', '1000');
        res = await fetch(`${sbUrl}/rest/v1/checkins?${s1.toString()}`, { headers: { apikey: sbKey, Authorization: 'Bearer ' + sbKey } });
      }
      if (res && res.ok) {
        const allCheckins = await res.json().catch(() => []);
        // Filtrer côté client par date
        checkins = allCheckins.filter(c => {
          const raw = c.created_at || c.timestamp || c.checkin_time;
          if (!raw) return false;
          const dt = new Date(raw);
          if (isNaN(dt.getTime())) return false;
          const checkinTime = dt.getTime();
          return checkinTime >= from.getTime() && checkinTime <= to.getTime();
        });
      }
    } catch { }
    ...
    try {
      // Utiliser le client Supabase JS si disponible, sinon construire la requête correctement
      if (window.supabase && typeof window.supabase.from === 'function') {
        try {
          const fromDateStr = toIsoDate(from);
          const toDateStr = toIsoDate(to);
          ...
          if (!planError && Array.isArray(plans)) {
            for (const pl of plans) {
              const day = (pl.date || '').toString().slice(0, 10);
              if (day) plannedDaysSet.add(day);
              let addedHours = 0;
              const hoursField = Number(pl.planned_hours ?? pl.estimated_hours);
              if (Number.isFinite(hoursField) && hoursField > 0) {
                addedHours = hoursField;
              } else if (pl.planned_start_time && pl.planned_end_time && day) {
                try {
                  ...
                  if (Number.isFinite(diffHours) && diffHours > 0) {
                    addedHours = diffHours;
                  }
                } catch (err) {
                  console.warn('Erreur calcul heures planifiées:', err);
                }
              }
              plannedHoursTotal += addedHours;
            }
          }
        } catch (sbError) {
          console.warn('Erreur requête Supabase JS planifications:', sbError);
        }
      }

      // Fallback: requête REST API sans filtres de date complexes (on filtrera côté client)
      if (plannedDaysSet.size === 0) {
        const p = new URLSearchParams();
        p.set('select', '*');
        ...
        if (resPlan.ok) {
          ...
          const filteredPlans = plans.filter(pl => {
            const day = (pl.date || '').toString().slice(0, 10);
            return day && day >= fromDateStr && day <= toDateStr;
          });

          for (const pl of filteredPlans) {
            const day = (pl.date || '').toString().slice(0, 10);
            if (day) plannedDaysSet.add(day);
            let addedHours = 0;
            const hoursField = Number(pl.planned_hours ?? pl.estimated_hours);
  ...
  try {
    ...
    if (window.offlineManager) {
      ...
      if (totalRemaining === 0) {
        ...
        const successMessage = totalUnsynced > 0
          ? `${totalUnsynced} enregistrement(s) synchronisé(s) avec succès`
          : 'Synchronisation terminée : aucune donnée en attente';
        ...
        if (typeof showNotification === 'function') {
          showNotification('Certaines données n\'ont pas pu être synchronisées. Réessayez.', 'warning');
        }
...
// Configuration du rafraîchissement automatique des métriques
setInterval(async () => {
  try {
...
setInterval(async () => {
  ...
  window.fetch = async function(resource, options = {}) {
    ...
    const publicPaths = ['/index.html', '/', '/login.html', '/forgot-password.html', '/reset-password.html'];
    
    // Si pas de token et qu'on n'est pas sur une page publique
    if (!token) {
      if (!publicPaths.some(path => currentPath.endsWith(path))) {
        console.warn('Aucun token valide trouvé, redirection vers la page de connexion');
        window.location.href = '/index.html';
        return false;
      }
      return true;
    ...
    // Vérifier la validité du token
      try {
        ...
        if (payload.exp && payload.exp < now) {
          ...
          const newToken = await window.refreshTokenIfNeeded(token);
...
async function checkAuth() {
  try {
    ...
    // Si on a un gestionnaire de session, on met à jour la session
    if (window.sessionManager) {
      const session = window.sessionManager.getSession();
      if (!session || session.token !== token) {
        await window.sessionManager.saveSession({ token });
      }
    }
...
(async function checkAuthOnLoad() {
  try {
    ...
    if (!isAuthenticated && !publicPaths.some(path => currentPath.endsWith(path))) {
      console.warn('Utilisateur non authentifié, redirection vers la page de connexion');
      window.location.href = '/index.html?session=expired';
      return;
    }
    
    // Démarrer le rafraîchissement automatique si authentifié
    if (isAuthenticated && window.sessionManager) {
      const session = window.sessionManager.getSession();
      if (session?.token) {
        window.sessionManager.startAutoRefresh();
    ...
    // En cas d'erreur, rediriger vers la page de connexion si nécessaire
    const currentPath = window.location.pathname;
    const publicPaths = ['/index.html', '/', '/login.html', '/forgot-password.html', '/reset-password.html'];
...
// Fonction globale pour rafraîchir le token si nécessaire
window.refreshTokenIfNeeded = async function(token) {
  if (!token) {
    console.log('🔍 Aucun token fourni pour le rafraîchissement');
    return null;
  }
  ...
  try {
    ...
    // Si le token est encore valide pour plus de 5 minutes, on ne fait rien
    ...
    const data = await response.json();
    if (data && data.token) {
      // Stocker le nouveau token dans le localStorage ET sessionStorage
      localStorage.setItem('jwt', data.token);
...
function initializeApp() {
  ...
  if (token) {
    ...
    window.refreshTokenIfNeeded(token);
  }
}
