...
for (const [type, data] of Object.entries(groupedRows)) {
  console.log(`\n=== ${type.toUpperCase()} (${data.length} items) ===`);
...
if (groupedRows.village) {
  for (const row of groupedRows.village) {
    const name = String(row.name || '').trim();
    const parent = String(row.admin || '').trim();
    if (name && parent) {
...
// Exporter en JSON
const fs = require('fs');
fs.writeFileSync('hierarchy-data.json', JSON.stringify(hierarchy, null, 2));
console.log('\nData exported to hierarchy-data.json');
